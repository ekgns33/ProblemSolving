...
public class P17837 {
  ...
  private static boolean move(Horse horse) {
    ...
    if (isOutOfBounds(nx, ny) || board[nx][ny] == 2) {
      ...
      moveStack(horse,x, y, nx, ny, true);
    } else if (board[nx][ny] == 0) {
      moveStack(horse, x, y, nx, ny, false);
    }
    return piles[nx][ny].size() >= 4;
  }

  private static void moveStack(Horse horse, int x, int y, int nx, int ny, boolean reverse) {
    List<Horse> stack = piles[x][y];
    List<Horse> toMove = new ArrayList<>();
